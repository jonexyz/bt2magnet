...
class BDecode  extends Base {
    private function numberdecode($wholefile, $offset) {
    ...
    private function decodeList($wholefile, $offset) {
        if ($wholefile[$offset] != 'l')
            return array(false);
        $offset++;
        $ret = array();
    ...
    private function decodeDict($wholefile, $offset=0) {
        if ($wholefile[$offset] == 'l')
            return $this->decodeList($wholefile, $offset);
        if ($wholefile[$offset] != 'd')
            return false;
    ...
    /**
     * @param $wholefile string 读取的bt种子的内容
     * @return mixed
     */
    public static function BDecodeDo($wholefile)
    {
        $decoder = self::getInstance();
